...
/// </summary>
public sealed class TrainControlService(ILegoService lego)
{
    ...
    public Task<IReadOnlyList<DiscoveredHub>> DiscoverAsync(CancellationToken ct = default) => lego.DiscoverAsync(ct);
    ...
    {
        var key = HubKey(train);
        return string.IsNullOrWhiteSpace(key) ? Task.CompletedTask : lego.DisconnectAsync(key, ct);
    }
    ...
    public Task SetSpeedAsync(TrainConfig train, int speed, CancellationToken ct = default)
    {
        ...
        return string.IsNullOrWhiteSpace(key)
            ? Task.CompletedTask
            : lego.SetSpeedAsync(key, MotorPort, (sbyte)Math.Clamp(speed, -100, 100), ct);
    }

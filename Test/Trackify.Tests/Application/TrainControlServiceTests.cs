...
public class TrainControlServiceTests
{
    ...
    {
        Assert.Equal("dev-1", TrainControlService.HubKey(new Train { HubId = "dev-1", BleAddress = "AA:BB" }));
        Assert.Equal("AA:BB", TrainControlService.HubKey(new Train { HubId = "", BleAddress = "AA:BB" }));
    }

    [Fact]
    public void IsSameDevice_matches_by_id_or_mac()
    {
        var train = new Train { HubId = "dev-1", BleAddress = "AA:BB:CC" };
    ...
    public async Task SetSpeedAsync_clamps_and_targets_the_motor_port()
    {
        ...
        await service.SetSpeedAsync(new Train { HubId = "dev-1" }, 150);

...
- `Trackify.Cli` — a **Spectre.Console.Cli** console app (single `net10.0`) to deploy on a Linux server / Raspberry Pi (drives hubs via BlueZ). Builds/runs on Windows too for dev/test, but without hub control off-Linux. `AddLinuxLego` picks the real BlueZ transport vs. a no-op at **runtime** (`OperatingSystem.IsLinux()`).
